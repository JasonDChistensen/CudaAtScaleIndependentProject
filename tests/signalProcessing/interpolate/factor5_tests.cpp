...
TEST(FACTOR5, INTERPOLATE)
{
    std::vector<float> h_Input = readFile("./vectors/inputSignal5.bin");
    ASSERT_NE(h_Input.size(), 0u);
    //printf("h_Input.size: %lu\n", h_Input.size());

    std::vector<float> h_filter = readFile("./vectors/interpFilter5.bin");
    ...
    std::vector<float> h_matlabInterpolatedOutput = readFile("./vectors/matlabInterpolatedOutput5.bin");
